...
void function () {
  var async, attached, auth, battery, config, EventEmitter, mainEndpoint, network, noErr, password, Photon, reboot, request, reset, router, schema, status, system, time, validate, wifi;
  async = require('async');
        ...
        return function () {
          this$.status(args, function (err, status) {
            ...
            if (!(null != err)) {
              ...
              if (ID_WIFI_SSID !== config.wifi.ssid)
